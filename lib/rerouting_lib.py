...
def compute_gst_sat_distance(altitude, min_elev, gst_pos, sat_tree):
    ...
    for id, cur in enumerate(sat_gst_dist):
        ...
        cos_elev = ((altitude + EARTH_RADIUS) * np.sin(cur)) / dist
        elev = np.rad2deg(np.arccos(cos_elev))
        elev_list.append(elev)
        projection = cos_elev * dist
        projection_list.append(projection)
    sat_gst_dist = np.asarray(dist_list)
    gst_elev_list = np.asarray(elev_list)
    projection_list = np.asarray(projection_list)

    return sat_gst_ind, sat_gst_dist , gst_elev_list , projection_list
...
if __name__ == "__main__":
    ...
    from plot_util import scatter_on_map
    ...
    # Load GSTS
    ixps = pd.read_csv("/ccr-submission-code/data/raw/ixp_geolocation.csv")
    ...
    # Load SRC positions
    cities = pd.read_csv("/ccr-submission-code/data/raw/WUP2018-F22-Cities_Over_300K_Annual.csv")
