...
class Constellation():
    ...
    def experiment_setup(self,sat_pos, altitude, src_pos, gst_pos, min_elev, orbits,
            ...
            sat_gst_ind, sat_gst_dist = compute_gst_sat_distance(altitude, min_elev,
             ...
             # compute 3 nearest gsts and their distances
            src_gst_ind, src_gst_dist = src_nearest_gst_distance(src_pos, gst_pos,
                                                                  path_control) #path_control = nn =3
 
             # compute gst-gst terrestrial distances
            gst_gst_terrestrial = gst_gst_terrestrial_distance(terrestrial_gst_graph,
                                                                gst_pos)
 
             # compute gst-gst distances(gst-sat+sat-sat)
    ...
    def run(self,path_control, deactivate_schedule, n_runs, cores, out_path,
        inactive_list=None):
        altitude = 1300
        min_elev = 40
        orbits = 32
    ...
    # Pre-compute intermediate results
        out = self.experiment_setup(sat_pos, altitude, src_pos, gst_pos, min_elev,
    ...
    # Run the comparison and save results
        ...
        if inactive_list is not None:
            ...
            with Pool(cores) as proc_pool:
                proc_pool.map(self.parallel_run_w_inactive, args)
    ...
    def parallel_run_w_inactive(self,args):
        inactive, cur_split, params = args
        for cur_num, cur_inactive in zip(cur_split, inactive):
            src_dst_lat_rerouting, src_dst_lat_pathcontrol = self.run_configuration(
                params['n_src'],
                cur_inactive,
                ...
                params['src_gst_ind'],
                params['src_gst_dist'])
            self.save_results(params['out_path'], cur_inactive, src_dst_lat_rerouting,
                     src_dst_lat_pathcontrol, run_n=cur_num)
    def parallel_run(self,args):
        self.sum_deactivate, cur_split, params = args
    ...
    def run_configuration(self,n_src, inactive, gst_gst_satellite, gst_gst_terrestrial,
        ...
        nearest_active, nearest_active_dist = inactive_to_closest_active(inactive, gst_gst_terrestrial)
	...
	# Latency matrix for path control ------------------------------------------
        src_dst_lat_pathcontrol = self.path_control_latency(n_src, inactive,
							   nearest_active,
							   nearest_active_dist,
							   gst_gst_satellite,
							   src_gst_ind, src_gst_dist)
    ...
    def path_control_latency(self,n_src, inactive, nearest_active, nearest_active_dist,
    ...
    def save_results(self,out_path, inactive, src_dst_lat_rerouting,
			 src_dst_lat_pathcontrol, run_n):
        dirname = f"compare_{len(inactive)}_inactive_{run_n}"
        new_path = os.path.join(out_path, dirname)
        os.mkdir(new_path)
    ...
    def matrices_to_loss_histogram(self,paco_latency, rero_latency):
...
if __name__ =="__main__":
    ...
    for t in range(0,10):
        StarA = Constellation()
        a = StarA.rotation_state()
        b = StarA.compute()

	#StarA.compute_latency()
	#StarA.avalibility()
	#StarA.save()
